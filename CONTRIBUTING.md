...
```bash
git clone https://github.com/go-rotini/jsonschema.git
cd jsonschema
go mod download
...
```bash
make test              # run tests
make test-acceptance   # run acceptance tests against real-world schemas
make test-bench        # run benchmarks
make test-conformance  # run JSON Schema Test Suite conformance tests
...
Open an issue with a minimal reproducing schema, instance, and the expected vs. actual behavior.
